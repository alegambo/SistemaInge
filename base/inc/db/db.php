 ...
 */
//seleccion utilizada para la dataTable base
...
//        if ($aActions["sAdd"] != '') {
//            $row[] = "<a class='puntero' title='A&ntilde;adir' onClick = 'javascript:" . $aActions["sAdd"] . '("' . $aRow[$sIndexColumn] . '","' . $aRow['Nom_Proy'] . '","' . $aRow['Cod_Ban'] . '","' . $aRow['Ent_Ad'] . '","' . $aRow['Num_Cue'] . '","' . $aRow['Fech_Ini'] . '","' . $aRow['Fech_Fin'] . '","' . $aRow['Estado'] . '"' . ");'><div class='text-center'><i class='fa fa-save text-success' title='Generar Reporte'></i></div></a>";
//        }
        $output[] = $row;
    }
    return $output;
}
 //seleccion utilizada para la dataTable de archivos
function seleccion_tabla2($sQuery, $sIndexColumn, $aColumns, $aActions) {
    require 'bdcommon.inc';
    $id_con = mysqli_connect($db_host, $usuario, $clave, $db);
    mysqli_set_charset($id_con, "utf8");
    $rResult = mysqli_query($id_con, $sQuery);
    ...
    while ($aRow = mysqli_fetch_array($rResult)) {
        ...
        if ($aActions["sEdit"] != '') {
            $row[] = "<a class='puntero'  data-toggle='modal' href='#modalPDF' data-target='#modalPDF' title='Editar' onClick = 'javascript:" . $aActions["sEdit"] . '("' . $aRow[$sIndexColumn] . '","' . $aRow['titulo'] . '","' . $aRow['descripcion'] .'","' . $aRow['cod_sia'] .'"'.  ");'><div class='text-center'><i class='fa fa-pencil text-success' title='Editar'></i></div></a>";
            //$row[] = "<a class='puntero'  data-toggle=''modal' href='#modalPDF' data-target='#modalPDF' title='Editar' onClick = 'javascript:" . $aActions["sEdit"] . '("' . $aRow[$sIndexColumn] . '"' . ");'><div class='text-center'><i class='fa fa-pencil text-success' title='Editar'></i></div></a>";
        }
        if ($aActions["sDele"] != '') {
            $row[] = "<a class='puntero'  title='Eliminar' onClick = 'javascript:" . $aActions["sDele"] . '("' . $aRow[$sIndexColumn] . '","' . $aRow['cod_sia'] . '","' . $aRow['nombre_archivo'] .'"'. ");'><div class='text-center'><i class='fa fa-close text-danger' title='Eliminar'></i></div></a>";
        }
        if ($aActions["sAdd"] != '') {
...
function seleccion_tabla_t($sQuery, $sIndexColumn, $aColumns, $aActions) {
    ...
    while ($aRow = mysqli_fetch_array($rResult)) {
        ...
        if ($aActions["sDele"] != '') {
            $row[] = "<a class='puntero' title='Eliminar' onClick = 'javascript:" . $aActions["sDele"] . '("' . $aRow[$sIndexColumn] . '","' . $aRow['Cod_SIA'] . '"' .  ");'><div class='text-center'><i class='fa fa-close text-danger' title='Eliminar'></i></div></a>";
        }
        if ($aActions["sAdd"] != '') {
            $row[] = "<a class='puntero' title='A&ntilde;adir' onClick = 'javascript:" . $aActions["sAdd"] . '("' . $aRow[$sIndexColumn] . '"' . ");'><div class='text-center'><i class='fa fa-pluss text-success' title='A&ntilde;adir'></i></div></a>";
        }
...
function seleccion_tabla_miembros_proyecto($sQuery, $sIndexColumn, $aColumns, $aActions) {
    require 'bdcommon.inc';
    $id_con = mysqli_connect($db_host, $usuario, $clave, $db);
    mysqli_set_charset($id_con, "utf8");
    $rResult = mysqli_query($id_con, $sQuery);
...
function seleccion_tabla_miembros_totales($sQuery, $sIndexColumn, $aColumns, $aActions) {
    require 'bdcommon.inc';
    $id_con = mysqli_connect($db_host, $usuario, $clave, $db);
    mysqli_set_charset($id_con, "utf8");
    $rResult = mysqli_query($id_con, $sQuery);
...
function seleccion_tabla_Reporte_pProy($sQuery, $sIndexColumn, $aColumns, $aActions) {
 require 'bdcommon.inc';
    $id_con = mysqli_connect($db_host, $usuario, $clave, $db);
    mysqli_set_charset($id_con, "utf8");
    $rResult = mysqli_query($id_con, $sQuery);
    ...
    while ($aRow = mysqli_fetch_array($rResult)) {
        ...
        if ($aActions["sPrin"] != '') {
            $row[] = "<a class='puntero' title='Imprimir' onClick = 'javascript:" . $aActions["sPrin"] . '("' . $aRow[$sIndexColumn] . '","' . $aRow['Nom_Proy'] . '"' . ");'><div class='text-center'><i class='fa fa-print text-primary' title='Imprimir'></i></div></a>";
        }
        $output[] = $row;
    }
    return $output;
