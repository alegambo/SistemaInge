...
$sAjaxSource = $mySessionController->getVar('cds_domain') . $mySessionController->getVar('cds_locate') . 'mod/admin/proyecto/ajax_listar_integrantes_totales.php';
?>

<div class="well well-sm"><h1>Lista de Miembros Totales</h1></div>
<?php if($user_rol == 4 ){ ?>

<script type="text/javascript" charset="utf-8">
    var asInitVals = new Array();   //Se utiliza para almacenar la llave del filtro para cada columna.
    jQuery(document).ready(function () {
        var oTable = jQuery('#lista_miembros_totales').dataTable({
        //"sPaginationType": "full_numbers",
         ...
         "aoColumns": [
            ...
            {"bVisible": false},
            null
        <?php if (check_permiso($mod4, $act1, $user_rol)) { ?> //act1 accion ver 'aoColumns es //info de columnas'
            , {"bSortable": false}
        <?php } ?>
        <?php if (check_permiso($mod4, $act4, $user_rol)) { ?> //act4 accion editar
            , {"bSortable": false}
        <?php } ?>
        <?php if (check_permiso($mod4, $act5, $user_rol)) { ?> //act5 accion eliminar
            , {"bSortable": false}
        <?php } ?>
                ],
...
<div class="dataTables_wrapper form-inline dt-bootstrap">
    <table id="lista_miembros_totales" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable" >
        <thead>
            <tr>
                <th>Cédula</th>
                <th>Nombre</th>
    ...
    jQuery(document).ready(function () {
        ...
        //"sPaginationType": "full_numbers",
         ...
         "aoColumns": [
            ...
            {"bVisible": false},
            null
        <?php if (check_permiso($mod2, $act1, $user_rol)) { ?> //act1 accion ver 'aoColumns es //info de columnas'
            , {"bSortable": false}
        <?php } ?>
        <?php if (check_permiso($mod2, $act4, $user_rol)) { ?> //act4 accion editar
            , {"bSortable": false}
        <?php } ?>
        <?php if (check_permiso($mod2, $act5, $user_rol)) { ?> //act5 accion eliminar
            , {"bSortable": false}
        <?php } ?>
                ],
         "sAjaxSource": "<?php echo $sAjaxSource; ?>"
