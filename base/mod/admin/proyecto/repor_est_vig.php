...
$sql1 = "SELECT DISTINCT Cod_SIA from vis_repor_proy_vig;";
$sql2 = "SELECT COUNT(DISTINCT (cod_SIA)) from vis_repor_proy_vig;";
...
for ($i = 0; $i < $res2[0]['COUNT(DISTINCT (cod_SIA))']; $i++) {
 //selecciona cada codigo SIA, obtenido desde la vista
    ...
    $sql3 = 'SELECT * FROM vis_repor_proy_vig WHERE Cod_SIA ="'.$res[$i]['Cod_SIA'].'";';
    $res3 = seleccion($sql3);
    $sql4 = 'SELECT COUNT(DISTINCT (Cod_SIA)) FROM vis_repor_proy_vig WHERE Cod_SIA ="'.$res[$i]['Cod_SIA'].'";';
    $res4 = seleccion($sql4);
    ...
    $sql6 = 'SELECT Ced, Nom_Miem, T_miembro, Jornada FROM vis_repor_proy_vig WHERE Cod_SIA ="'.$res[$i]['Cod_SIA'].'";';
    $res5 = seleccion($sql6);
    $sql7 = 'SELECT COUNT(*) FROM vis_repor_proy_vig WHERE Cod_SIA ="'.$res[$i]['Cod_SIA'].'";';
    $res6 = seleccion($sql7);
